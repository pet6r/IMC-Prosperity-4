...
EMERALD_BAND = 5.0
EMERALD_ZSCORE_ENABLED = False
EMERALD_EWMA_ALPHA = 0.06
EMERALD_SIGMA_FLOOR = 1.0
EMERALD_Z_CLIP = 2.5
EMERALD_Z_TO_PRICE = 0.9

# At |position| == limit, fair is shifted by ±this many price units (short -> +skew -> buy higher / sell higher).
INVENTORY_SKEW_MAX = 4.0
...
# Stateful EMERALDS moments for z-score fair estimate.
_emerald_mu: float | None = None
_emerald_var: float = EMERALD_SIGMA_FLOOR * EMERALD_SIGMA_FLOOR
...
def _popular_mid(state: TradingState, symbol: str) -> float | None:
...
def _fair_emeralds(state: TradingState) -> float:
    mid = _popular_mid(state, "EMERALDS")
    if mid is None:
        return EMERALD_ANCHOR
    if EMERALD_ZSCORE_ENABLED:
        return _fair_emeralds_zscore(mid)
    if (EMERALD_ANCHOR - EMERALD_BAND) <= mid <= (EMERALD_ANCHOR + EMERALD_BAND):
        return EMERALD_ANCHOR
    return mid


def _fair_emeralds_zscore(mid: float) -> float:
    """
    Mean-revert around anchor with z-score from EWMA moments.
    Positive z (rich) lowers fair; negative z (cheap) raises fair.
    """
    global _emerald_mu, _emerald_var

    if _emerald_mu is None:
        _emerald_mu = mid
        _emerald_var = EMERALD_SIGMA_FLOOR * EMERALD_SIGMA_FLOOR
        return EMERALD_ANCHOR

    sigma = _sqrtf(max(_emerald_var, EMERALD_SIGMA_FLOOR * EMERALD_SIGMA_FLOOR))
    z = (mid - _emerald_mu) / sigma if sigma > 0 else 0.0
    z = max(-EMERALD_Z_CLIP, min(EMERALD_Z_CLIP, z))
    fair = EMERALD_ANCHOR - EMERALD_Z_TO_PRICE * z

    alpha = EMERALD_EWMA_ALPHA
    new_mu = (1.0 - alpha) * _emerald_mu + alpha * mid
    resid = mid - new_mu
    _emerald_var = (1.0 - alpha) * _emerald_var + alpha * (resid * resid)
    _emerald_mu = new_mu
...
def _fair_tomatoes(state: TradingState) -> float:
    mid = _popular_mid(state, "TOMATOES")
